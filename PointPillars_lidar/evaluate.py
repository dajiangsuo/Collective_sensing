...
from dataset import lidar, get_dataloader
...
def do_eval(det_results, gt_results, CLASSES, saved_path):
    ...
    overall_results = {}
    eval_types=['bbox_bev', 'bbox_3d']
    for e_ind, eval_type in enumerate(eval_types):
...
def main(args):
    val_dataset = lidar.LIDAR(data_root=args.data_root,
                        split=args.val)
    val_dataloader = get_dataloader(dataset=val_dataset, 
                                    batch_size=args.batch_size, 
                                    num_workers=args.num_workers,
                                    shuffle=False)
    CLASSES = lidar.LIDAR.CLASSES
    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(tqdm(val_dataloader)):
            ...
            for j, result in enumerate(batch_results):
                ...
                write_label(format_result, os.path.join(saved_submit_path, idx+'.txt'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', default='pillar_logs/checkpoints/epoch_last.pth', help='your checkpoint for kitti')
    ...
    parser.add_argument('--val', default='val', help='file for val')

...
def create_data_info_pkl1(data_root, data_type, prefix, label=True, db=False):
    sep = os.path.sep
...
def create_data_info_pkl(data_root, data_type, prefix, label=True, db=False):
    ...
    if db:
        ...
        os.makedirs(db_points_saved_path, exist_ok=True)
    for id in tqdm(ids):
        yaml_annotation_dict={}
        if label:
            label_path = os.path.join(data_root, split, 'lidar', f'{id}.yaml')
            yaml_annotation_dict = read_label(label_path)

        for i in range(4):
            cur_info_dict={}
            lidar_path = os.path.join(data_root, split, 'lidar', f'{id}_lidar{i}.pcd')
            #calib_path = os.path.join(data_root, split, 'calib', f'{id}.txt') 
            cur_info_dict['lidar'] = sep.join(lidar_path.split(sep)[-3:])

            lidar_points = read_points(lidar_path)
          
            if label:
                # label_path = os.path.join(data_root, split, 'lidar', f'{id}.txt')
                # annotation_dict = read_label(label_path)
                annotation_dict=copy.copy(yaml_annotation_dict)
                annotation_dict['difficulty'] = np.zeros(len(annotation_dict['name']))#judge_difficulty(annotation_dict)
                annotation_dict['num_points_in_gt'] = get_points_num_in_bbox(
                    points=lidar_points,
                    r0_rect=None, 
                    tr_velo_to_cam=None,
                ...
                kitti_infos_dict[f'{id}_lidar{i}'] = cur_info_dict
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prefix', default='lidar', 

...
def main(args):
    setup_seed()
    train_dataset = lidar.LIDAR(data_root=args.data_root,
                          split='train')
    ...
    for epoch in range(args.max_epoch):
        ...
        with torch.no_grad():
            for i, data_dict in enumerate(tqdm(val_dataloader)):
                ...
                val_step += 1
        pointpillars.train()
        torch.save(pointpillars.state_dict(), os.path.join(saved_ckpt_path, f'epoch_last.pth'))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Configuration Parameters')
    parser.add_argument('--data_root', default='./', 
                        help='your data root for lidar')
